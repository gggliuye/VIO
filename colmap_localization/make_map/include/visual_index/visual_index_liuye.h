#ifndef LIU_YE_VISUAL_INDEX_H
#define LIU_YE_VISUAL_INDEX_H

//#include <iostream>
//#include <chrono>

#include "retrieval/visual_index.h"
#include "visual_index/inverted_index_liuye.h"
...
class VisualIndex_LY {
public:
    static const int kMaxNumThreads = -1;
    typedef InvertedIndex_LY<kDescType, kDescDim, kEmbeddingDim> InvertedIndexType;
...
VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::VisualIndex_LY()
    : prepared_(false) {}
...
template <typename kDescType, int kDescDim, int kEmbeddingDim>
size_t VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::NumVisualWords() const {
  return visual_words_.rows;
}

template <typename kDescType, int kDescDim, int kEmbeddingDim>
void VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::Add(
    ...
    for (typename DescType::Index i = 0; i < descriptors.rows(); ++i) {
        ...
        // for each of the near word (word_id)
...
template <typename kDescType, int kDescDim, int kEmbeddingDim>
bool VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::ImageIndexed(
    const int image_id) const {
  return image_ids_.count(image_id) != 0;
}

template <typename kDescType, int kDescDim, int kEmbeddingDim>
void VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::Prepare() {
...
template <typename kDescType, int kDescDim, int kEmbeddingDim>
void VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::Query(
...
// Modified by LIU.YE
template <typename kDescType, int kDescDim, int kEmbeddingDim>
void VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::QueryAndFindWordIds(
    const QueryOptions& options, const DescType& descriptors,
    std::vector<ImageScore>* image_scores, Eigen::MatrixXi* word_ids) const {

    // check if words are well prepared
    CHECK(prepared_);

    // if input none descriptor
    if (descriptors.rows() == 0) {
        image_scores->clear();
        return;
    }
    ...
    *word_ids = FindWordIds(descriptors, options.num_neighbors,
    ...
    //std::cout << "   find word ids time : " << ttrack << std::endl << std::endl;

    inverted_index_.Query(descriptors, *word_ids, image_scores);

    //std::chrono::steady_clock::time_point t3 = std::chrono::steady_clock::now();
    //ttrack= std::chrono::duration_cast<std::chrono::duration<double> >(t3 - t2).count();
    //std::cout << "   inverted index query time : " << ttrack << std::endl << std::endl;
    ...
    //std::cout << "   sort scores time : " << ttrack << std::endl << std::endl;

}

// Modified by LIU.YE
template <typename kDescType, int kDescDim, int kEmbeddingDim>
Eigen::MatrixXi VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::FindWordIds(
    ...
    // init output ids, then convert to flann type
    ...
    // the distance of each output neighbors, then convert to flann type
    ...
    // conver the descriptor into word queue
...
template <typename kDescType, int kDescDim, int kEmbeddingDim>
void VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::Quantize(
...
template <typename kDescType, int kDescDim, int kEmbeddingDim>
void VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::Build(
    const BuildOptions& options, const DescType& descriptors) {
  // Quantize the descriptor space into visual words.
...
template <typename kDescType, int kDescDim, int kEmbeddingDim>
void VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::Read(
  ...
  // Read the inverted index.
  {
    std::ifstream file(path, std::ios::binary);
    CHECK(file.is_open()) << path;
    // seek from beg(begin) and have "file_offset" offset 
...
template <typename kDescType, int kDescDim, int kEmbeddingDim>
void VisualIndex_LY<kDescType, kDescDim, kEmbeddingDim>::Write(
    const std::string& path) {
  // Write the visual words.
  {
    ...
    CHECK(file.is_open()) << path;
    // convert the data type to Little Endian then write
  ...
  {
    // app means to add at the end of the exist file (if file exist)
