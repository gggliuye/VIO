...
class KNNResultSet:
    def __init__(self, capacity):
        # the parameter k (#points to search)
        self.capacity = capacity
        self.count = 0
        # initialize the worst distance as infinite
        self.worst_dist = 1e10
        # inistailize the list
    ...
    def add_point(self, dist, index):
        ...
        while i > 0:
            if self.dist_index_list[i-1].distance > dist:
                # use deepcopy to avoid memory error
                # move the element backwards to make space for the new comer
