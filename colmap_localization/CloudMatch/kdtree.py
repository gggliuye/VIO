...
# Node class : the basic element of a tree
class Node:
    def __init__(self, axis, value, left, right, point_indices):
        # divide axis of this node
        self.axis = axis
        self.value = value
        self.left = left
        self.right = right
        # the indices of points rest in the node(leaf)
        self.point_indices = point_indices

    # the flag for leaf is:: whether the value is None
    def is_leaf(self):
...
# function : build the kd-tree recursively
# input:
#     root: root of the tree
#     db: data to save
#     point_indices：sorted indices
#     axis: scalar
#     leaf_size: scalar
# output:
#     root: the built tree
def kdtree_recursive_build(root, db, point_indices, axis, leaf_size):
    ...
    # build a leaf if point size smaller than the leaf size
    # determine whether to split into left and right
    if len(point_indices) > leaf_size:
        # --- get the split position --- by sorting all the points in the node
        # the cost of sorting the points is high
        point_indices_sorted, _ = sort_key_by_vale(point_indices, db[point_indices, axis])  # M
        # split the points by the #points
        ...
        # the right part of the points
        middle_right_idx = middle_left_idx + 1
        middle_right_point_idx = point_indices_sorted[middle_right_idx]
        middle_right_point_value = db[middle_right_point_idx, axis]

        # root value : is the value of the division (mean of the middle left and middle right points)
