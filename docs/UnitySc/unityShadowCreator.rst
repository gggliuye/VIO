...
8.4 两个系统的松耦合
>>>>>>>>>>>>>>>>>>>>>>>>
由于在这样的框架中，我们使用的是两个系统的松耦合，松耦合就会导致信息的缺失，从而导致难以修正的误差。下面我将具体分析：

由于两个系统给出的结果都不是绝对准确的，但是在这个项目中，我们简单的把两个系统看成两个黑箱子（black box），这就导致原本是误差信息，被抹杀了。
系统本身的结果是 **预测值** 和 **误差信息** （实际的表现形式可能多种多样），但是被下一个黑盒子取用的只有 **预测值** （数学上反映可以举例为：从高斯函数变成了狄拉克函数），在这样的处理下，信息的损失是永久的。
