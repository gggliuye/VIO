...
void ViewerLY::testpangolin()
{
    ...
    pangolin::Var<bool> menuShowArcoreFrames("menu.Show Arcore Frames",true,true);
...
void ViewerLY::DrawArcoreframes()
{
    const float &w = mKeyFrameSize*1.5;
    const float h = w*0.75;
    const float z = w*0.6;
    
    for(size_t i = 0; i < vArCores.size(); i++)
    {
        glPushMatrix();

#ifdef HAVE_GLES
            glMultMatrixf(vArCores[i].m);
#else
            glMultMatrixd(vArCores[i].m);
#endif
...
void ViewerLY::SetArcorePose(Eigen::Quaterniond quaternion, Eigen::Vector3d tvec)
{
    //Eigen::Quaterniond quaternion(qvec(0),qvec(1),qvec(2),qvec(3));
    ...
    currentMatrix.block(0,0,3,3) = quaternion.matrix();
    currentMatrix(0,3) = tvec(0);
    currentMatrix(1,3) = tvec(1);
    currentMatrix(2,3) = tvec(2);
