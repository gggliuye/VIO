...
void Estimator::setParameter()
{
    ...
    project_sqrt_info_ = FOCAL_LENGTH / 15 * Matrix2d::Identity();
    td = TD;
}
...
Eigen::Matrix<double, 3, 4> Estimator::GetCurrentCameraPose()
{
    ...
    Qd qic(para_Ex_Pose[0][6], para_Ex_Pose[0][3], para_Ex_Pose[0][4], para_Ex_Pose[0][5]);
    Vector3d tic(para_Ex_Pose[0][0], para_Ex_Pose[0][1], para_Ex_Pose[0][2]);

    Matrix3d rotate = Rs[WINDOW_SIZE] * qic.matrix();    //  Rs is Rwi
    Vector3d pose = Rs[WINDOW_SIZE] * tic + Ps[WINDOW_SIZE];
...
// retrive the data from the vector to double array
...
// retrive the optimized parameters as ouput
...
void Estimator::problemSolve()
{
    ...
    //std::cout << "IMU covarianve" << std::endl;
    // IMU
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        int j = i + 1;
        ...
        //std::cout << pre_integrations[j]->covariance*1000000 << std::endl << std::endl;

    }

    //std::cout << "Visual covarianve" << std::endl;
    //std::cout << project_sqrt_info_.transpose() * project_sqrt_info_ << std::endl;
    // Visual Factor
    ...
    runtimefile << problem.solvetime << " " << problem.hessiantime << " " << problem.shurtime/problem.shurnumber<< std::endl;
...
{
    TicToc t_solver;
    // use vins framework，maintain the variables
    vector2double();
    // build the solver
    problemSolve();
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        std::unordered_map<long, double *> addr_shift; // the parameters' addresses keeped in prior 
